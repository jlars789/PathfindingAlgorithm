...
void printPath(){                         //draws the path described by the path array
...
void printLine(){    //printing a single contigous path from start to finish                //XX(fixed):not working, stuck in infinite loop with while loop, parent path might be broken
...
void move(){                                      //executes the pathfinding algorithm
...
int[] findNextLowest(){                                 //choosing another head position due to previous head having higher neighbors
...
int[] searchThroughNL(ArrayList a){    //chooses among equidistant heuristic values
  int[] pos = new int[2];
  int num =0;
  int min = Integer.MAX_VALUE;
...
void fillbuffer(){                            //calculates the heurisitc distance values around the location head
